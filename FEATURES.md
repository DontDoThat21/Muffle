...
| 9.2 | Subscription model (premium) | ✅ | UserSubscription model (Free/Premium/Premium+ tiers), UserSubscriptions table (SQLite + SQL Server), SubscriptionService (subscribe/cancel/feature comparison), SubscriptionView with tier cards + feature table, Premium button in sidebar |
...
| 9.7 | Picture-in-picture mode (mobile) | ✅ | InCallBanner (friend name, duration, mute, minimize, end) + PipPanel overlay (bottom-right, LIVE badge, Restore/End buttons); `CallStateChanged` event drives banner show/hide; 1-second timer for live duration display; mute toggle; minimize/restore between banner and PiP panel |
